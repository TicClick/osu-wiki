---
stub: true
tags:
  - train
outdated_translation: true
outdated_since: 4559e4a54b5c5fe76dfb41652b74b6d9d2cca80b
---

# Hype

[Pending](/wiki/Beatmap/Category#wip-and-pending)状態の[ビートマップ](/wiki/Beatmap)には**Hype train** というプレイヤーのビートマップへの関心度を示し[Ranked](/wiki/Beatmap/Category#ranked)の状態にするための物がある。[ビートマップディスカッション](/wiki/Beatmap_discussion)の`一般(全ての難易度)`で`Hype!` を押すことによりビートマップのHype trainをためることができる。

## Rankedにするには

Hype trainが少なくとも5つ(メモリの数)たまらなければビートマップをRankedにするための[ノミネーション](/wiki/Beatmap_ranking_procedure#nominations)を受けることができない、つまりこれこそがその第一歩となる。

## 一人があげれる上限

ユーザーは10個Hypeを持っており、ビートマップにHypeをするとHypeした分消費する。Hypeは消費した1週間後に回復する。
